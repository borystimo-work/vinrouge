...
document.addEventListener("click", (e) => {
  const favorite = e.target.closest(".footer-swiper-slide__favorite");
  if (!favorite) return;
...
function initSliders() {
  if (document.querySelector(".best-spirit__slider")) {
    new Swiper(".best-spirit__slider", {
      ...
      // rewind: true, // вместо loop
      // loopAdditionalSlides: 5,
      // Пагінація
      pagination: {
        el: ".best-spirit-pagination",
        clickable: true
      },
      // Кнопки "вліво/вправо"
      navigation: {
        prevEl: ".best-spirit__arrow--prev",
        nextEl: ".best-spirit__arrow--next"
      },
      // Брейкпоінти
      breakpoints: {
        320: {
  ...
  if (document.querySelector(".quotes__slider")) {
  ...
  (function() {
    ...
    LightGallery2.prototype.loadContent = function(index, rec) {
      ...
      if (!$currentSlide.hasClass("lg-loaded")) {
        ...
        this.LGel.trigger(lGEvents$1.afterAppendSlide, { index });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (e) => {
    ...
    const card = e.target.closest(".cardflip");
    if (!card) {
      if (activeCard) {
        isAnimating = true;
        const inner = activeCard.querySelector(".cardflip__inner");
    ...
    if (card === activeCard) {
      isAnimating = true;
      const inner = card.querySelector(".cardflip__inner");
    ...
    isAnimating = true;
    const newInner = card.querySelector(".cardflip__inner");
    const oldInner = activeCard?.querySelector(".cardflip__inner");
